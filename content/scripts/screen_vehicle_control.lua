...
function update(screen_w, screen_h, ticks)
    ...
    if g_screen_index == 0 then
        ...
        if screen_vehicle:get() then
            -- render following map mfd
            if screen_vehicle:get_definition_index() ~= e_game_object_type.chassis_carrier then
                g_is_mfd = 1
                g_camera_pos_x = screen_vehicle:get_position_xz():x()
                g_camera_pos_y = screen_vehicle:get_position_xz():y()
                g_viewing_vehicle_id = screen_vehicle:get_id()
                g_camera_size = 10000

                -- zoom out/in to first waypoint if we have one
                local waypoint_count = screen_vehicle:get_waypoint_count()
                if waypoint_count > 0 then

                    local wp1 = screen_vehicle:get_waypoint(0)
                    if wp1:get() then
                        local wp1_pos = wp1:get_position_xz(0)
                        local v_x = screen_vehicle:get_position_xz():x()
                        local v_y = screen_vehicle:get_position_xz():y()

                        local dist_x = math.abs(wp1_pos:x() - v_x)
                        local dist_y = math.abs(wp1_pos:y() - v_y)
                        local wp_dist = 2 * math.sqrt(dist_x * dist_x + dist_y * dist_y)
                        if wp_dist > 10000 then
                            g_camera_size = 10000
                        end
                        if wp_dist < 7500 then
                            g_camera_size = wp_dist + 200
                        end
                        if wp_dist < 200 then
                            g_camera_size = 200
                        end
                    ...
                    if vehicle_definition_index ~= e_game_object_type.chassis_spaceship and vehicle_definition_index ~= e_game_object_type.drydock then
                        ...
                        if is_visible and is_revealed then
                            ...
                            if is_render_vehicle_icon then
                                ...
                                if g_is_mfd == 1 or vehicle:get_definition_index() == e_game_object_type.chassis_carrier then
                                    if not g_is_mfd then
