...
--        end
    ...
    if update_get_is_notification_holomap_set() then
        ...
        for i = 0, vehicle_count - 1, 1 do
            ...
            if vehicle_team == screen_team and vehicle_definition_index ~= e_game_object_type.chassis_sea_barge and vehicle_definition_index ~= e_game_object_type.drydock then
                ...
                if vehicle_support_id ~= 0 then
                    ...
                    -- Draw velocity ghosts
                    --local v_velocity = vehicle:get_linear_velocity()
                    --local v_speed = vehicle:get_linear_speed()
                    --if v_speed > 1 then
                    --    --print(string.format("%d", v_speed))
                    --end
        ...
        if g_is_ruler then
            ...
            update_ui_text(g_pointer_pos_x + 15, g_pointer_pos_y,
                    string.format("%.0f deg", bearing_world), 100, 0, text_col, 0)


            cy = cy - 10

            local dist = vec2_dist(vec2(g_ruler_x, g_ruler_y), vec2(world_x, world_y))
            update_ui_text(g_pointer_pos_x + 15, g_pointer_pos_y + 9,
                    string.format("%.1f km", dist / 1000), 100, 0, text_col, 0)
